...
if (platform === 'darwin' || platform === 'freebsd' || platform === 'freebsd') {
  ...
  execSync(
    `npx json -I -f package.json -e 'this.resolutions={"**/fsevents": "^2.1.2"}'`,
    { encoding: 'utf-8' },
    display
  )
} else {
...
const adblockerPathFull = path.resolve(
  'packages',
...
if (major >= 8) {
  ...
  for (const module of modules) {
    try {
      execSync(`yarn remove @types/puppeteer`, {
        cwd: path.resolve('packages', module),
        encoding: 'utf-8'
      })
    } catch (e) {}
  }
  ...
  execSync(
    `yarn lerna add --dev @types/puppeteer`,
    { encoding: 'utf-8' },
    display
  )
...
fs.copyFileSync(
  path.join(tsDir, `puppeteer.ts.${mode}`),
  path.join(tsDir, 'puppeteer.ts')
)
prntLog(`activating puppeteer.ts${mode}`, `Done`)

prntLog(`Change @cliqz/adblocker-puppeteer version to ${cliqz}`, `Start`)
execSync(
  `yarn add @cliqz/adblocker-puppeteer@${cliqz}`,
  { cwd: adblockerPathFull, encoding: 'utf-8' },
  display
)
...
execSync(
  `yarn lerna add --dev puppeteer@${version}`,
  { encoding: 'utf-8' },
  display
)
