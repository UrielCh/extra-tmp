...
const utils = {
    ...
    traps.forEach(trap => {
      newHandler[trap] = function (...args: any[]) {
        try {
          ...
          const stripWithBlacklist = (stack: string, stripFirstLine = true) => {
            const blacklist = [
            ...
            return (
              err.stack
                ...
                .filter((line: string) => !blacklist.some(bl => line.trim().startsWith(bl)))
                .join('\n')
            )
          }
          ...
          const stripWithAnchor = (stack: string, anchor?: string) => {
            ...
            const anchorIndex = stackArr.findIndex(line =>
              line.trim().startsWith(anchor as string)
            )
            if (anchorIndex === -1) {
   ...
   */
  stripErrorWithAnchor: (err: Error, anchor: string) => {
    const stackArr = (err.stack as string).split('\n')
    const anchorIndex = stackArr.findIndex((line: string) => line.trim().startsWith(anchor))
    if (anchorIndex === -1) {
   ...
   */
  replaceProperty: (obj: any, propName: string, descriptorOverrides = {}) => {
   ...
   */
  replaceWithProxy: (obj: any, propName: string, handler: ProxyHandler<any>) => {
   ...
   * @param {object} handler - The JS Proxy handler to use
   */
  mockWithProxy: (obj: any, propName: string, pseudoTarget: any, handler: any) => {
   ...
   * @param {object} handler - The JS Proxy handler to use
   */
  createProxy: (pseudoTarget: any, handler: any) => {
    const proxyObj = new Proxy(pseudoTarget, utils.stripProxyFromErrors(handler))
    utils.patchToString(proxyObj)
   ...
   * @param {object} handler - The JS Proxy handler to use
   */
  replaceObjPathWithProxy: (objPath: string, handler: any) => {
   ...
   */
  execRecursively: (obj = {} as any, typeFilter = [] as string[], fn: Function) => {
   ...
   */
  stringifyFns: (fnObj: any = { hello: () => 'world' }) => {
    ...
    return (Object.fromEntries || fromEntries)(
      Object.entries(fnObj)
        ...
        .map(([key, value]) => [key, (value as Function).toString()]) // eslint-disable-line no-eval
    )
  },
