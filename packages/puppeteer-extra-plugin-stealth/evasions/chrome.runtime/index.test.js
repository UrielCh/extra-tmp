...
var window;
var chrome;
(0, ava_1.default)('vanilla: is chrome false', async (t) => {
    const pageFn = async (page) => await page.evaluate(() => window.chrome); // eslint-disable-line
    const { pageFnResult: chrome, hasChrome } = await (0, util_1.getVanillaFingerPrint)(pageFn);
    t.is(hasChrome, false);
    t.false(chrome instanceof Object);
    t.is(chrome, undefined);
});
(0, ava_1.default)('stealth: is chrome true', async (t) => {
    const pageFn = async (page) => await page.evaluate(() => window.chrome); // eslint-disable-line
    const { pageFnResult: chrome, hasChrome } = await (0, util_1.getStealthFingerPrint)(_1.default, pageFn);
    t.is(hasChrome, true);
    t.true(chrome instanceof Object);
});
(0, ava_1.default)('stealth: will add convincing chrome.runtime object', async (t) => {
    const puppeteer = (0, util_2.addExtra)(util_2.vanillaPuppeteer).use((0, _1.default)({
    ...
    const results = await page.evaluate(() => {
        ...
        return {
            ...
            sendMessage: {
                ...
                validIdWorks: chrome.runtime.sendMessage('nckgahadagoaajjgafhacjanaoiihapd', '') ===
                    undefined
            },
            sendMessageErrors: {
                noArg: catchErr(chrome.runtime.sendMessage),
                singleArg: catchErr(chrome.runtime.sendMessage, ''),
                tooManyArg: catchErr(chrome.runtime.sendMessage, '', '', '', '', '', ''),
...
(0, ava_1.default)('stealth: will add convincing chrome.runtime.connect', async (t) => {
    const puppeteer = (0, util_2.addExtra)(util_2.vanillaPuppeteer).use((0, _1.default)({
        runOnInsecureOrigins: true // for testing
    }));
    const browser = await puppeteer.launch({ headless: true });
    ...
    const results = await page.evaluate(() => {
        ...
        return {
            connect: {
                ...
                validIdWorks: chrome.runtime.connect('nckgahadagoaajjgafhacjanaoiihapd') !==
                    undefined
            },
            connectErrors: {
...
(0, ava_1.default)('stealth: will add convincing chrome.runtime.connect response', async (t) => {
    const puppeteer = (0, util_2.addExtra)(util_2.vanillaPuppeteer).use((0, _1.default)({
        runOnInsecureOrigins: true // for testing
    }));
    const browser = await puppeteer.launch({ headless: true });
    const page = await browser.newPage();
    const results = await page.evaluate(() => {
        const connectResponse = chrome.runtime.connect('nckgahadagoaajjgafhacjanaoiihapd');
        return {
            connectResponse: {
...
(0, ava_1.default)('stealth: error stack is fine', async (t) => {
    const puppeteer = (0, util_2.addExtra)(util_2.vanillaPuppeteer).use((0, _1.default)({
        runOnInsecureOrigins: true // for testing
    }));
    const browser = await puppeteer.launch({ headless: true });
